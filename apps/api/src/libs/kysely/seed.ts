...
export const dbSeed = async () => {
  ...
  await db.schema
    .createTable('galrc_comments')
    .ifNotExists()
    .addColumn('id', 'varchar(255)', (cb) => cb.notNull().primaryKey())
    .addColumn('targetType', 'varchar(255)', (cb) => cb.notNull())
    .addColumn('targetId', 'varchar(255)', (cb) => cb.notNull())
    .addColumn('userId', 'varchar(255)', (cb) => cb.notNull())
    .addColumn('content', 'text', (cb) => cb.notNull())
    .addColumn('type', 'varchar(255)', (cb) => cb.notNull())
    .addColumn('parentId', 'varchar(255)', (cb) => cb.notNull())
    .addColumn('rootId', 'varchar(255)')
    .addColumn('depth', 'integer', (cb) => cb.notNull())
    .addColumn('replyToUserId', 'varchar(255)')
    .addColumn('status', 'varchar(255)', (cb) => cb.notNull())
    .addColumn('feedbackStatus', 'varchar(255)')
    .addColumn('isPinned', 'boolean', (cb) => cb.notNull())
    .addColumn('isWhispers', 'boolean', (cb) => cb.notNull())
    .addColumn('lastReplyAt', 'timestamp')
    .addColumn('meta', 'jsonb')
    .addColumn('createdAt', 'timestamp', (cb) => cb.notNull())
    .addColumn('updatedAt', 'timestamp', (cb) => cb.notNull())
