...
const getSearch = createServerFn()
  ...
  .handler(async ({ data }) => {
    const gameListData = await api.search.get({
      query: { q: data.q || "", limit: 100, startDate: data.startDate ? format(parseISO(data.startDate), "yyyyMMdd") : undefined, endDate: data.endDate ? format(parseISO(data.endDate), "yyyyMMdd") : undefined },
    });
...
export const Route = createFileRoute("/search/")({
  ...
  loaderDeps: ({ search: { q, startDate, endDate } }) => ({ q, startDate, endDate }),
  loader: async ({ deps }) => {
