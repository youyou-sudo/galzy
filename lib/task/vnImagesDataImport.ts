...
const processInWorker = (ref: Ref, uptime: string) => {
  return new Promise<void>((resolve, reject) => {
    const workerPath = path.resolve(process.cwd(), "worker/imagesWorker.js");
    ...
    worker.on("message", async (result) => {
      await pushQueue(`TagQueue:${ref.id}`, JSON.stringify(result));
      processQueue();
    });
    ...
    async function processQueue() {
      ...
      let cachedData;
      while ((cachedData = await popQueue(`TagQueue:${ref.id}`)) !== null) {
      ...
      await prisma.duptimes.update({
        ...
        data: {
          updatetime: uptime,
          state: false,
          Statusdescription: "数据更新成功 imagesTack",
        },
      });

      await delKv(`TagQueue:${ref.id}`);
      resolve();
    }
...
// 操作函数
export const iamgesGet = async (ref: Ref, uptime: string) => {
  try {
    await prisma.duptimes.update({
      where: { id: ref.id },
